...
if __name__ == '__main__':
    ...
    X, y = datasets.make_regression(n_samples = 100, n_features = 2, noise = 30, random_state = 4)
    ...
    fitted_beta = np.linalg.inv(X_train.T @ X_train) @ X_train.T @ y_train

    ones_col = np.ones((X_test.shape[0], 1))
    X_test = np.concatenate([ones_col, X_test], axis=1)
    predict_ = X_test @ fitted_beta
